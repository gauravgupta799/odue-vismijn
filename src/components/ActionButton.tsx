...
export function ActionButton({children, dark = false, className, onClick,}: ActionButtonProps) {
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'group inline-flex h-[36px] items-stretch rounded-none border text-xs font-medium tracking-[-0.12px] transition-colors duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-current focus-visible:ring-offset-2 capitalize btn',
        dark 
        ? 'border-cream/80 bg-transparent text-cream hover:bg-cream hover:text-ink' 
        : 'border-ink bg-ink text-cream hover:bg-ink/85',
        className,
      )}
    >
      ...
      <span className={cn('flex h-full w-[33px] items-center justify-center border-l transition-colors duration-300', dark ? 'border-cream/80 group-hover:border-ink' : 'border-white', )}>
        <ArrowUpRightIcon className="h-4 w-4 btn-icon" strokeWidth={1.5} />
